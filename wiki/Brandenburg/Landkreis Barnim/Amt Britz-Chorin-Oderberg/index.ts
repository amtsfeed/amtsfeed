...
// ── Amtsblatt ─────────────────────────────────────────────────────────────────
// WordPress custom theme (abco). Each year in a .volume div.
// Published issues: <li class="issue issue--published">
//   <a class="issue__link" href="/amtsblatt/YYYY-NN-amtsblatt-bco.pdf">
//   <time class="issue__date" datetime="YYYY-MM-DD">

function extractAmtsblatt(html: string): AmtsblattItem[] {
  const items: AmtsblattItem[] = [];
  const now = new Date().toISOString();

  const blocks = html.split('<li class="issue issue--published">').slice(1);
  for (const block of blocks) {
    const hrefMatch = block.match(/href="(\/amtsblatt\/(\d{4})-(\d{2})-amtsblatt-bco\.pdf)"/);
    const dateMatch = block.match(/datetime="(\d{4}-\d{2}-\d{2})"/);
    if (!hrefMatch || !dateMatch) continue;

    const [, path, year, num] = hrefMatch;
    const publishedAt = `${dateMatch[1]}T00:00:00.000Z`;
    const id = `bco-amtsblatt-${year}-${num}`;
