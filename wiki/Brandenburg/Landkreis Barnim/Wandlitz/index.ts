...
const BASE_URL = "https://www.wandlitz.de";
...
function decodeHtmlEntities(str: string): string {
  return str
    ...
    .replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&bdquo;/g, "„").replace(/&ldquo;/g, "“").replace(/&rdquo;/g, "”").replace(/&ndash;/g, "–").replace(/&mdash;/g, "—")
...
function extractEvents(html: string): Event[] {
  ...
  for (const block of blocks) {
    ...
    // site uses <h6> or <h2 class="legacy_h6"> depending on CMS version
    ...
    // endDate from <time datetime="YYYY-MM-DD"> — skip 1970 placeholders
...
function extractNews(html: string): NewsItem[] {
  ...
  for (const block of blocks) {
    ...
    // wandlitz.de uses news-entry-new-4-date (seelow.de uses news-entry-new-3-date)
...
function extractNotices(html: string): NoticeItem[] {
  ...
  while ((m = rowRx.exec(html)) !== null) {
    ...
    const id = `wandlitz-notice-${slug || title.toLowerCase().replace(/[^a-z0-9]+/g, "-").slice(0, 40)}`;
...
assertAllowed(robots, ["/veranstaltungen/", "/news/", "/amtsblatt/index.php", "/bekanntmachungen/"]);
...
const mergedAmtsblatt = mergeAmtsblatt(existingAmtsblatt.items, extractAmtsblatt(amtsblattHtml, AMTSBLATT_URL, "wandlitz"));
