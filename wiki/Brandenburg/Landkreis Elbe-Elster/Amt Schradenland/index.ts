...
const BASE_URL = "https://www.amt-schradenland.de";
...
// PortUNA events-entry-3: <time class="events-entry-3-time" datetime="YYYY-MM-DD">
// Headline: class="events-entry-3-headline"
function extractEvents(html: string): Event[] {
  const events: Event[] = [];
  const now = new Date().toISOString();
  // Each row: split by events-entry-3-time-wrapper
  const blocks = html.split(/(?=events-entry-3-time-wrapper)/).filter((b) => b.includes("events-entry-3-headline"));
  const seen = new Set<string>();
  for (const block of blocks) {
    // Date from <time class="events-entry-3-time" datetime="YYYY-MM-DD">
    const dateMatch = block.match(/class="events-entry-3-time"[^>]*datetime="(\d{4}-\d{2}-\d{2})"/);
    if (!dateMatch) continue;
    const isoDate = dateMatch[1]!;
    // Link with ID: /veranstaltungen/ID/...
    const linkMatch = block.match(/href="(\/veranstaltungen\/(\d+)\/[^"]+)"/);
    ...
    // Title
    const titleMatch = block.match(/class="[^"]*events-entry-3-headline[^"]*"[^>]*>[\s\S]*?<a[^>]*>([\s\S]*?)<\/a>/i);
    if (!titleMatch) continue;
    const title = decodeHtmlEntities((titleMatch[1] ?? "").replace(/<[^>]+>/g, "").trim());
    if (!title) continue;
    events.push({ id, title, url, startDate: `${isoDate}T00:00:00.000Z`, fetchedAt: now, updatedAt: now });
  }
  return events;
}
