...
const BASE_URL = "https://www.herzberg-elster.de";
...
function extractEvents(html: string): Event[] {
  ...
  // PortUNA "new-2" tile layout: <div class="event-entry-new-2">
  const blocks = html.split('<div class="event-entry-new-2">').slice(1);
  for (const block of blocks) {
    // Link with date in path: /veranstaltungen/ID/YYYY/MM/DD/slug.html
    ...
    // Title in <a class="event-entry-new-2-headline ..."> or <h2/h3>
    const titleMatch = block.match(/class="event-entry-new-2-headline[^"]*"[^>]*>[\s\S]*?<a[^>]*>([\s\S]*?)<\/a>/i);
    if (!titleMatch) continue;
    const title = decodeHtmlEntities((titleMatch[1] ?? "").replace(/<[^>]+>/g, "").trim());
    if (!title) continue;
    // Time from event-entry-new-2-daytime: "HH:MM Uhr"
    const timeMatch = block.match(/class="event-entry-new-2-daytime"[^>]*>([\s\S]*?)<\/div>/i);
...
function extractNews(html: string): NewsItem[] {
  ...
  for (const block of blocks) {
    ...
    items.push({ id, title, url, ...(description ? { description } : {}), fetchedAt: now, ...(publishedAt ? { publishedAt } : { publishedAt: null }), updatedAt: now });
  }
  return items;
}
