#!/usr/bin/env tsx
// Sonnewalde: www.sonnewalde.de redirects to www.stadt-sonnewalde.de (PortUNA CMS)
...
const BASE_URL = "https://www.stadt-sonnewalde.de";
...
// PortUNA classic event-box layout
function extractEvents(html: string): Event[] {
  ...
  for (const block of blocks) {
    ...
    const titleMatch = block.match(/class="event-title">\s*<a[^>]*>([\s\S]*?)<\/a>/i);
    if (!titleMatch) continue;
    const title = decodeHtmlEntities((titleMatch[1] ?? "").replace(/<[^>]+>/g, "").trim());
    if (!title) continue;
    const timeMatch = block.match(/class="event-time">([\s\S]*?)<\/span>/i);
...
// PortUNA news-entry-new layout
function extractNews(html: string): NewsItem[] {
  const items: NewsItem[] = [];
  const now = new Date().toISOString();
  const rx = /href="(\/news\/\d+\/(\d+)\/[^"]+)"[^>]*>[\s\S]*?<h3[^>]*>([\s\S]*?)<\/h3>/g;
  ...
  while ((m = rx.exec(html)) !== null) {
    ...
    if (!title) continue;
    const url = href.startsWith("http") ? href : `${BASE_URL}${href}`;
    items.push({ id, title, url, publishedAt: null, fetchedAt: now, updatedAt: now });
  }
  return items;
}
