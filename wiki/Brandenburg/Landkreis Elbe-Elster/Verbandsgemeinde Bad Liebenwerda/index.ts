...
const BASE_URL = "https://www.verbandsgemeinde-liebenwerda.de";
...
// ── News ──────────────────────────────────────────────────────────────────────
// PortUNA news-entry-to-limit: date in <p class="vorschau">DD.&#8203;MM.&#8203;YYYY: ...</p>
...
function extractNews(html: string): NewsItem[] {
  ...
  for (const block of blocks) {
    ...
    const id = `vg-liebenwerda-news-${idMatch ? idMatch[1] : encodeURIComponent(href).slice(0, 50)}`;
    ...
    if (vorschauMatch) {
      ...
      if (dm) publishedAt = `${dm[3]}-${dm[2]}-${dm[1]}T00:00:00.000Z`;
    }
...
// ── Events ────────────────────────────────────────────────────────────────────
// PortUNA event-entry-new-1: date from URL /veranstaltungen/{ID}/{YYYY}/{MM}/{DD}/{slug}.html
...
function extractEvents(html: string): Event[] {
  ...
  while ((m = rx.exec(html)) !== null) {
    ...
    const id = `vg-liebenwerda-event-${eventId!}-${yyyy}${mm}${dd}`;
    if (seen.has(id)) continue;
    seen.add(id);
...
// ── Amtsblatt ─────────────────────────────────────────────────────────────────
// Table: <td>Nr. X/YYYY</td><td>DD.&#8203;MM.&#8203;YYYY</td>
...
function extractAmtsblatt(html: string): AmtsblattItem[] {
  ...
  const rx = /<tr>\s*<td>(Nr\.[^<]+)<\/td>\s*<td>([\d&#;.]+)<\/td>/gi;
  let m: RegExpExecArray | null;
  while ((m = rx.exec(html)) !== null) {
    const title = `Amtsblatt ${decodeHtmlEntities((m[1] ?? "").trim())}`;
    const dateText = decodeHtmlEntities((m[2] ?? "").trim());
    const dm = dateText.match(/^(\d{2})\.(\d{2})\.(\d{4})$/);
    if (!dm) continue;
    const publishedAt = `${dm[3]}-${dm[2]}-${dm[1]}T00:00:00.000Z`;

    const id = `vg-liebenwerda-amtsblatt-${dm[3]}-${dm[2]}-${title.replace(/[^a-z0-9]/gi, "-").toLowerCase()}`;
    if (seen.has(id)) continue;
    seen.add(id);
...
// ── Bekanntmachungen ──────────────────────────────────────────────────────────
// PortUNA announcement table: <td valign="top">DD.MM.YYYY</td><td valign="top">Title...</td><td><a href="...">
...
function extractNotices(html: string): NoticeItem[] {
  ...
  // Try class="table-title" first, fallback to valign="top" pattern
  const rx = /<tr[^>]*>\s*<td[^>]*(?:class="table-title"|valign="top")[^>]*>([\s\S]*?)<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>\s*(?:<td[^>]*>([\s\S]*?)<\/td>)?/gi;
  let m: RegExpExecArray | null;
  while ((m = rx.exec(html)) !== null) {
    ...
    const id = `vg-liebenwerda-notice-${dm[3]}-${dm[2]}-${dm[1]}-${title.slice(0, 40).replace(/[^a-z0-9]/gi, "-").toLowerCase()}`;
    if (seen.has(id)) continue;
    seen.add(id);
