#!/usr/bin/env tsx
/**
 * Scraper for Amt Nennhausen (PortUNA / VerwaltungsPortal CMS).
 * https://www.amt-nennhausen.de
 *
 * News:           /news/1                          — news-entry-to-limit / news-entry-new-2 (Titel in <h3>)
 * Events:         /veranstaltungen/index.php       — event-entry-new-* (Datum aus URL)
 * Amtsblatt:      /amtsblatt/index.php             — Tabelle Nr./Datum/Form (PDF via POST)
 * Bekanntmachungen:/bekanntmachungen/index.php     — derzeit leer ("Es sind leider keine Daten vorhanden.")
 */
...
const SLUG = "amt-nennhausen";
const BASE_URL = "https://www.amt-nennhausen.de";
...
// Bekanntmachungen-Seite vorhanden, aber regelmäßig leer ("Es sind leider keine Daten vorhanden.").
// Wir behalten existierende Einträge bei (falls jemals welche da waren), schreiben aber keine neuen.
const noticesEmpty = noticesHtml.includes("Es sind leider keine Daten vorhanden") || !/<td[^>]*class="table-title"/i.test(noticesHtml);
const existingNotices: NoticeItem[] = loadJson<NoticesFile>(noticesPath, { updatedAt: "", items: [] }).items;
const mergedNotices: NoticeItem[] = noticesEmpty ? existingNotices : existingNotices;
...
console.log(`notices:    ${mergedNotices.length} Einträge → ${noticesPath} (Quelle leer)`);
