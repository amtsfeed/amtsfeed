#!/usr/bin/env tsx
/**
 * Scraper for Amt Rhinow (PortUNA / VerwaltungsPortal CMS).
 * https://www.rhinow.de   (KEIN amt-rhinow.de — die Verwaltung publiziert unter rhinow.de)
 *
 * News:           /news/1                          — news-entry-to-limit / news-entry-new-2 (Titel in <h3>)
 ...
 * Bekanntmachungen:/bekanntmachungen/index.php     — Tabelle <td>DD.MM.YYYY</td><td>Titel</td><td><a pdf></td>
 */
...
const SLUG = "amt-rhinow";
const BASE_URL = "https://www.rhinow.de";
...
function extractNews(html: string): NewsItem[] {
  ...
  for (const block of blocks) {
    ...
    if (dateBlock) {
      const parsed = parseGermanDate(stripTags(dateBlock[1] ?? ""));
      if (parsed) publishedAt = parsed;
    }
    // Rhinow-Fallback: Datum als Präfix im Vorschau-Absatz, z.B. "<p class=\"vorschau\">04.05.2026: ..."
    if (!publishedAt) {
      const previewMatch = block.match(/<p[^>]*class="vorschau[^"]*"[^>]*>\s*(\d{1,2}\.\d{1,2}\.\d{4})/i);
      if (previewMatch) {
        const parsed = parseGermanDate(previewMatch[1]!);
        if (parsed) publishedAt = parsed;
      }
    }
...
// Rhinow-spezifisch: <tr><td valign="top">DD.MM.YYYY</td><td valign="top">Titel</td><td><a href="...pdf">…</a></td></tr>
function extractNotices(html: string): NoticeItem[] {
  const items: NoticeItem[] = [];
  const now = new Date().toISOString();
  const seen = new Set<string>();

  // Pro Bekanntmachung gibt es eine eigene <table class="break-word"> (gruppiert nach Jahr/Monat).
  // Alle solchen Tabellen einsammeln und in jeder die Datenzeilen extrahieren.
  const tableRe = /<table[^>]*class="break-word"[^>]*>([\s\S]*?)<\/table>/gi;
  const scopes: string[] = [];
  let tm: RegExpExecArray | null;
  while ((tm = tableRe.exec(html)) !== null) scopes.push(tm[1]!);
  const scope = scopes.join("\n");

  const rowRe = /<tr>\s*<td[^>]*>([^<]+)<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>\s*<\/tr>/gi;
  let m: RegExpExecArray | null;
  while ((m = rowRe.exec(scope)) !== null) {
    ...
    const linkBlock = m[3] ?? "";
    const linkMatch = linkBlock.match(/<a[^>]+href="([^"]+)"/i);
