#!/usr/bin/env tsx
/**
 * Scraper for Stadt Falkensee (PortUNA / VerwaltungsPortal CMS).
 * https://www.falkensee.de
 *
 * News:           /news/rss.xml                     — RSS-Feed (zuverlässiger als HTML)
 * Events:         /veranstaltungen/index.php        — event-entry-new-2 (Datum aus URL)
 * Amtsblatt:      /amtsblatt/index.php              — Tabelle Nr./Datum/Form (PDF via POST)
 * Bekanntmachungen: keine Inhalte unter /bekanntmachungen/index.php
 */
...
const SLUG = "falkensee";
const BASE_URL = "https://www.falkensee.de";
...
function extractNewsFromRss(xml: string): NewsItem[] {
  ...
  while ((m = itemRe.exec(xml)) !== null) {
    ...
    // Titel beginnt häufig mit "DD.MM.YYYY: "; abschneiden
...
// ── Amtsblatt ─────────────────────────────────────────────────────────────────
// Tabellenformat: <td>Nr. N/YYYY</td><td>DD.MM.YYYY</td><td>...gazette_ID...</td>
...
const [newsXml, eventsHtml, amtsblattHtml] = await Promise.all([
