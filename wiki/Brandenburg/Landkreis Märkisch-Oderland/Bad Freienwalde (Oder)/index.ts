...
function extractEvents(html: string): Event[] {
  ...
  for (const block of blocks) {
    ...
    const startDate = `${parseGermanDate(d1!, m1!, y1!)}T${time ? `${time}:00.000Z` : "00:00:00.000Z"}`;
    const endDate = d2 ? `${parseGermanDate(d2, m2!, y2!)}T${time ? `${time}:00.000Z` : "00:00:00.000Z"}` : undefined;
