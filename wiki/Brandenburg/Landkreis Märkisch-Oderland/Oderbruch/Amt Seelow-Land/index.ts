...
const BASE_URL = "https://www.amt-seelow-land.de";
...
function extractEvents(html: string): Event[] {
  ...
  for (const block of blocks) {
    ...
    const h6Match = block.match(/<h6>\s*<a[^>]*>([\s\S]*?)<\/a>\s*<\/h6>/i);
    if (!h6Match) continue;
    ...
    const endDate = validDates.at(-1) ? `${validDates.at(-1)}T00:00:00.000Z` : undefined;
...
function extractNews(html: string): NewsItem[] {
  ...
  for (const block of blocks) {
    const h5Match = block.match(/<h5>\s*<a\s+href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/i);
    if (!h5Match) continue;

    const href = h5Match[1]!;
    const title = decodeHtmlEntities((h5Match[2] ?? "").replace(/<[^>]+>/g, "").trim());
    if (!title) continue;
    ...
    // ID is the numeric part of the URL: /news/1/ID/nachrichten/...
    ...
    items.push({
      ...
      // No date on this site — use fetchedAt as RSS pubDate fallback
      fetchedAt: now,
      updatedAt: now,
    });
  }
...
function mergeNews(existing: NewsItem[], incoming: NewsItem[]): NewsItem[] {
  ...
  // Sort by numeric ID descending (higher = newer)
...
const mergedAmtsblatt = mergeAmtsblatt(existingAmtsblatt.items, extractAmtsblatt(amtsblattHtml, AMTSBLATT_URL, "seelow-land"));
