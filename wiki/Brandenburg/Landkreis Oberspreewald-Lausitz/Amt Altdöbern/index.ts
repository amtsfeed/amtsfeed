#!/usr/bin/env tsx
/**
 * Scraper for Amt Altdöbern — PortUNA (verwaltungsportal.de)
 * https://www.amt-altdoebern.de
 *
 * Events: PortUNA event-box variant.
 * News:   PortUNA news-entry-to-limit.
 * Amtsblatt: PortUNA table (Nr. X/YYYY | DD.MM.YYYY).
 */
...
const BASE_URL = "https://www.amt-altdoebern.de";
...
function extractEvents(html: string): Event[] {
  ...
  for (const block of blocks) {
    ...
    const id = `${ID_PREFIX}-event-${datePathMatch[1]!}`;
