...
const BASE_URL = "https://www.calau.de";
const SLUG = "calau";
...
function extractAmtsblatt(html: string): AmtsblattItem[] {
  ...
  const now = new Date().toISOString();
  const seen = new Set<string>();
  // Allow suffix letter after issue number (e.g. "Nr. 5a/2026")
  const rx = /<td>(Nr\.\s*(\d+[a-z]?)\/(\d{4}))<\/td>\s*<td>([\d.&#; ]+)<\/td>/g;
  let m: RegExpExecArray | null;
  while ((m = rx.exec(html)) !== null) {
    const num = m[2]!;
    const year = m[3]!;
    ...
    if (seen.has(id)) continue;
    seen.add(id);
    items.push({ id, title: `Amtsblatt Nr. ${num}/${year}`, url: AMTSBLATT_URL, publishedAt, fetchedAt: now });
  }
  return items.sort((a, b) => b.publishedAt.localeCompare(a.publishedAt));
}

// Calau notices use table-title format:
...
function extractNotices(html: string): NoticeItem[] {
  ...
  const now = new Date().toISOString();
  const rowRx = /<tr\s+valign="top">\s*<td class="table-title">\s*([\d.&#;\s]+?)\s*<\/td>\s*<td[^>]*>\s*([\s\S]*?)\s*<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>\s*<\/tr>/g;
  let m: RegExpExecArray | null;
  let counter = 0;
  while ((m = rowRx.exec(html)) !== null) {
    const dateStr = m[1]!.replace(/&#[^;]+;/g, "").replace(/\s+/g, "").trim();
    const dateParts = dateStr.match(/^(\d{1,2})\.(\d{1,2})\.(\d{4})$/);
    if (!dateParts) continue;
    const publishedAt = `${dateParts[3]}-${dateParts[2]!.padStart(2, "0")}-${dateParts[1]!.padStart(2, "0")}T00:00:00.000Z`;
  ...
  return items.sort((a, b) => b.publishedAt.localeCompare(a.publishedAt) || a.title.localeCompare(b.title));
}
