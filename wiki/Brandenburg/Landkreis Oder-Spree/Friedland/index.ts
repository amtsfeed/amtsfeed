...
const BASE_URL = "https://www.friedland-nl.de";
const EVENTS_URL = `${BASE_URL}/veranstaltungen/index.php`;
const NEWS_URL = `${BASE_URL}/news/index.php?rubrik=15`;
...
// ── Events ────────────────────────────────────────────────────────────────────
// friedland-nl.de uses <div class="event-field"> containers with ISO datetime attrs

function extractEvents(html: string): Event[] {
  const events: Event[] = [];
  const now = new Date().toISOString();

  const blocks = html.split(/(?=<div\s+class="event-field")/)
    .filter((b) => b.includes('class="event-field"'));
  ...
  for (const block of blocks) {
    ...
    // ISO date from <time class="event-time..." datetime="YYYY-MM-DD">
    ...
    // Start/end time from <time>HH:MM</time> inside event-time-start
    // Use a fixed-length slice after the class marker to avoid nested-tag boundary issues
    ...
    const startIso = times[0]
      ? startDate.replace("T00:00:00.000Z", `T${times[0]}:00.000Z`)
      : startDate;
    const endDate = times[1]
      ? startDate.replace("T00:00:00.000Z", `T${times[1]}:00.000Z`)
      : undefined;
...
// ── News ──────────────────────────────────────────────────────────────────────
// friedland-nl.de: <div class="news-entry-new ..."> with <time datetime="YYYY-MM-DD">
...
function extractNews(html: string): NewsItem[] {
  ...
  for (const block of blocks) {
    const hMatch = block.match(/<h[2-6][^>]*>([\s\S]*?)<\/h[2-6]>/i);
    if (!hMatch) continue;
    const title = decodeHtmlEntities((hMatch[1] ?? "").replace(/<[^>]+>/g, "").trim());
    if (!title) continue;

    const linkMatch = block.match(/href="(\/news\/[^"]+\.html)"/);
    if (!linkMatch) continue;
    const href = linkMatch[1]!;

    // ISO date directly from <time ... datetime="YYYY-MM-DD">
...
function extractAmtsblatt(html: string): AmtsblattItem[] {
  ...
  while ((m = rx.exec(html)) !== null) {
    ...
    items.push({
      id: `friedland-amtsblatt-${year}-${num}`,
...
function extractNotices(html: string): NoticeItem[] {
  ...
  while ((m = rowRx.exec(html)) !== null) {
    ...
    const id = tokenMatch
      ? `friedland-notice-${tokenMatch[1]!.replace(/\//g, "")}`
      : `friedland-notice-${publishedAt.slice(0, 10)}-${encodeURIComponent(title).slice(0, 40)}`;
