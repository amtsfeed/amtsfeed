...
const BASE_URL = "https://www.amt-brueck.de";
...
function extractEvents(html: string): Event[] {
  ...
  while ((m = rx.exec(html)) !== null) {
    ...
    events.push({ id: `amt-brueck-event-${eventId}`, title, url: `${BASE_URL}${href}`, startDate: startDateTime, ...(location ? { location } : {}), fetchedAt: now, updatedAt: now });
  }
  return events;
}
...
function extractAmtsblatt(html: string): AmtsblattItem[] {
  ...
  while ((m = rx.exec(html)) !== null) {
    ...
    items.push({ id: `amt-brueck-amtsblatt-${year}-${num}`, title: `Amtsblatt Nr. ${num}/${year}`, url: AMTSBLATT_URL, publishedAt, fetchedAt: now });
  }
  return items.sort((a, b) => b.publishedAt.localeCompare(a.publishedAt));
}
