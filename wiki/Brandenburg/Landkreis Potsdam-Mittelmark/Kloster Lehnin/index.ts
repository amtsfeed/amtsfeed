...
const BASE_URL = "https://www.klosterlehnin.de";
...
function extractEvents(html: string): Event[] {
  ...
  while ((m = rx.exec(html)) !== null) {
    ...
    events.push({ id: `kloster-lehnin-event-${eventId}`, title, url: `${BASE_URL}${href}`, startDate: startDateTime, ...(location ? { location } : {}), fetchedAt: now, updatedAt: now });
  }
  return events;
}
...
function extractAmtsblatt(html: string): AmtsblattItem[] {
  ...
  while ((m = rx.exec(html)) !== null) {
    ...
    items.push({ id: `kloster-lehnin-amtsblatt-${year}-${num}`, title: `Amtsblatt Nr. ${num}/${year}`, url: AMTSBLATT_URL, publishedAt, fetchedAt: now });
  }
  return items.sort((a, b) => b.publishedAt.localeCompare(a.publishedAt));
}
...
function extractNotices(html: string): NoticeItem[] {
  ...
  while ((m = rowRx.exec(html)) !== null) {
    ...
    const id = `kloster-lehnin-notice-${slug || title.toLowerCase().replace(/[^a-z0-9]+/g, "-").slice(0, 40)}`;
