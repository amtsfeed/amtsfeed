...
const BASE_URL = "https://www.teltow.de";
...
function extractEvents(html: string): Event[] {
  ...
  while ((m = rx.exec(html)) !== null) {
    ...
    events.push({ id: `teltow-event-${eventId}`, title, url: `${BASE_URL}${href}`, startDate: startDateTime, ...(location ? { location } : {}), fetchedAt: now, updatedAt: now });
  }
  return events;
}
